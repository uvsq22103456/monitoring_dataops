...
# Tes textes exacts avec les détails entre parenthèses
DOMAINES = {
    "Vente": "",
...
# --- FONCTIONS HTML ---
def generer_html_tableau(date, statuts, titre):
    lignes_html = ""
...
def generer_html_liste_ok(rapports, date):
    ...
    <div style="background-color: #f0f2f5; padding: 20px; font-family: Arial, sans-serif;">
        <div style="background-color: white; border-radius: 8px; padding: 15px; margin-bottom: 15px; border: 1px solid #e0e0e0;">
            <h2 style="margin: 0; color: #000;"><img src="{URL_LOGO}" height="35" style="vertical-align:middle;"> | Données du {date} Disponibles</h2>
        </div>
        <div style="background-color: white; border-radius: 8px; padding: 20px; border: 1px solid #e0e0e0; border-left: 5px solid #4CAF50;">
            <p style="font-weight: bold;">✅ Les rapports suivants ont été mis à jour et sont disponibles :</p>
            <ul>{liste}</ul>
...
if mode == "Tout OK ✅":
    ...
    if format_success == "Liste simple":
        rapports_selectionnes = st.multiselect("Sélectionnez les rapports vérifiés :", ["SUIVI VENTES UNITAIRES", "FLASH MARQUES PROPRES", "SUIVI DES STOCKS"], default=["SUIVI VENTES UNITAIRES", "FLASH MARQUES PROPRES", "SUIVI DES STOCKS"])
    else:
        for dom in DOMAINES.keys(): statuts_tableau[dom] = {"PBI": "✅ disponible", "Deci": "✅ disponible"}

elif mode == "Partiel ⚠️":
    rapports_selectionnes = st.multiselect("Sélectionnez les rapports KO :", ["SUIVI VENTES UNITAIRES", "FLASH MARQUES PROPRES", "SUIVI DES STOCKS"])

elif mode == "Retard Global 🚨":
    for domaine in DOMAINES.keys():
        col1, col2, col3 = st.columns([2, 1, 1])
        with col1: st.write(f"**{domaine}**")
        with col2: pbi = st.selectbox(f"PBI {domaine}", ["✅ disponible", "⚠️ en cours"], index=1, key=f"p_pbi_{domaine}", label_visibility="collapsed")
        with col3: deci = st.selectbox(f"Deci {domaine}", ["✅ disponible", "⚠️ en cours"], index=1, key=f"p_deci_{domaine}", label_visibility="collapsed")
...
if st.button("🚀 ENVOYER L'ALERTE", type="primary"):
    try:
        if mode == "Tout OK ✅":
            ...
            sujet, html = f"🟠 MYDATA : Partiellement disponible ({date_str})", generer_html_orange(rapports_selectionnes, date_str)
        else:
            sujet, html = f"🔴 MYDATA : Retard sur les Données du {date_str}", generer_html_tableau(date_str, statuts_tableau, "Retard sur les Données")
        ...
        st.success(f"✅ Alerte envoyée pour le {date_str} !")
